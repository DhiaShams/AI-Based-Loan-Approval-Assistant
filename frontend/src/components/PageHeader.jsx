...
export function Metric({ label, value, tone = '' }) {
  const hasWarning = tone === 'risk' || tone === 'high' || label.toLowerCase().includes('risk') || label.toLowerCase().includes('default');
  return (
    <div className={`metric-card ${tone}`}>
      <div className="metric-header">
        <span>{label}</span>
        {hasWarning && <AlertTriangle size={15} className="metric-warn-icon" />}
      </div>
      <strong>{value}</strong>
    </div>
  );
